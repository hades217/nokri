...
if ( $recent_job_query->have_posts() )
{
	  ...
	  { 
			...
			{ 
				$apply_button = '<a href="javascript:void(0)" class="absolute-position btn n-btn-flat external_apply" data-job-id="'. esc_attr( $job_id ).'"  data-job-exter="'.( $job_apply_url ).'">'. esc_html__('Apply now', 'nokri' ).'</a>';
			}
			else if ($job_apply_with == 'mail') 
			{
				$apply_button = '<a href="javascript:void(0)" class="absolute-position btn n-btn-flat external_apply" data-job-id="'. esc_attr( $job_id ).'" data-job-exter="'.( $job_apply_mail ).'">'. esc_html__('Apply now', 'nokri' ).'</a>';
			}
			else
			{
				$apply_button = '<a href="javascript:void(0)" class="absolute-position btn n-btn-flat apply_job" data-toggle="modal" data-target="#myModal"  data-job-id='.esc_attr( $job_id ).'>'.esc_html__( 'Apply Now', 'nokri' ).' </a>';
			}
...
if (!empty($authors))
{
	foreach ($authors as $author)
	{
		...
		if($cand_add != '')     {$cand_address = '<div class="n-candidate-lcation"><a href="javascript:void(0)"><i class="fa fa-map-marker"></i>'.$cand_add.'</a></div>';   }
		/* Getting Star*/									  
